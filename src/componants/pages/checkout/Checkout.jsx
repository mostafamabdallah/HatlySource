...
import { changeToken } from "../../../redux/tokenRedux";
...
const Checkout = () => {
  ...
  const getToken = () =>
    axios.post("https://accept.paymob.com/api/auth/tokens", {
      api_key:
        "",
    });
  ...
  const paymentCash = (data) => {
    ...
    getToken().then((response) => {
      sendOrder(response.data.token, data).then((res) => {
        getPaymentKeys(response.data.token, res.data.id, data).then(
          (paymentdata) => {
            cashRequest(paymentdata.data.token).then(() => {
              navigate("/payment_response");
            });
          }
        );
      });
                  ...
                  {cart.products.map((product, i) => {
                    return (
                      <div className="row align-items-center">
                        ...
                        <div className="col-8">
                          <p style={pStyle}>{product.title}</p>
                          <p
                            style={{
                              margin: "0",
                              color: "#20c997",
